...
def load_radar(example_path: AnyStr) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]:
    ...
    # TODO: fix this in the raw data
    azimuths = np.arange(0, raw_example_data[:, 8:10].shape[0], 1).reshape(raw_example_data[:, 8:10].shape[0], 1)
    azimuths = azimuths / float(raw_example_data[:, 8:10].shape[0]) * 2 * np.pi
